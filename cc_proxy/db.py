...
def db_set_circuit_config(provider_name: str, data: dict[str, Any]) -> dict[str, Any]:
    """设置 provider 熔断器配置（upsert via DELETE + INSERT for openGauss）"""
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def db_update_health(provider_name: str, data: dict[str, Any]) -> None:
    """更新 provider 健康状态（upsert via DELETE + INSERT for openGauss）"""
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def db_rollup_usage(retention_days: int = 30) -> int:
    ...
    try:
        ...
        cur.execute("""
            SELECT created_at::date as day, model_id, provider_name,
                   COUNT(*) as cnt,
                   COALESCE(SUM(input_tokens), 0),
                   COALESCE(SUM(output_tokens), 0),
                   COALESCE(SUM(cache_read_tokens), 0),
        ...
        for row in rollups:
            day, model_id, provider_name, cnt, inp, out, cr, cc = row
            cur.execute(
                "DELETE FROM usage_daily_rollups WHERE day = %s AND model_id = %s AND provider_name = %s",
                (day, model_id, provider_name)
            )
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (day, model_id, provider_name, cnt, inp, out, cr, cc))
    ...
    try:
        ...
        for key, value in settings.items():
            if key in ("migrated", "id", "passthrough_paths"):
                continue
            json_val = _json.dumps(value) if not isinstance(value, str) else value
        ...
        for k, v in model_map.items():
            cur.execute("INSERT INTO model_map (source, target) VALUES (%s, %s)", (k, v))
